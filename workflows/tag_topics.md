...
## Objective
Assign 1-3 canonical QA/testing topic labels to every item that doesn't have any yet,
using Claude.
...
## Tool
`tools/claude_tag_topics.py` — `tag(title, summary, existing_labels) -> list[str]`.
...
2. `tools/claude_tag_topics.py` returns 1-3 short topic label strings.
